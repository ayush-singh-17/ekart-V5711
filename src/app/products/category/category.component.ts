        ...
        for (var p = 0; p < this.cartItem.length; p++) {
          ...
          for (var q = 0; q < this.productItem.length; q++) {
            ...
            if (this.cartItem[p] == this.productItem[q].id) {
              const item = Object.assign({},this.productItem[q],{itemQuantity: 1})
              this.displayInCart.push(item);
             this.count+=1;
...
calculatePrice()
 {
   this.totalSum =0;
   this.subTotalSum =0;
   for(var i =0; i<this.displayInCart.length;i++){
     this.subTotalSum += this.displayInCart[i].price*this.displayInCart[i].itemQuantity;
    
   }
   this.totalSum = this.subTotalSum + this.shippingCharges;
 }
    }
